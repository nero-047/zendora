...
type PublicOrderInvoicePageProps = {
...
async function getInvoiceData(props: PublicOrderInvoicePageProps) {
...
export async function generateMetadata(
  props: PublicOrderInvoicePageProps,
): Promise<Metadata> {
  const data = await getInvoiceData(props);
  ...
  return {
    title: getStoreSeoTitle(data.store, `Invoice ${data.order.id.slice(0, 8)}`),
    robots: {
      index: false,
      follow: false,
    },
...
export default async function PublicOrderInvoicePage(
  props: PublicOrderInvoicePageProps,
) {
