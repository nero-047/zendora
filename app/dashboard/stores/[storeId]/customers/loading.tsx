  ...
  return (
    <div className="grid gap-5">
      ...
      <section className="dashboard-grid">
        {["customers", "repeat", "sales", "average"].map((item) => (
      ...
      <section className="soft-panel overflow-hidden">
        {[1, 2, 3].map((item) => (
          <div className="border-b border-slate-100 p-4 last:border-0" key={item}>
            <div className="h-4 w-48 animate-pulse rounded-[8px] bg-slate-200" />
            <div className="mt-3 h-3 w-64 max-w-full animate-pulse rounded-[8px] bg-slate-100" />
          </div>
        ))}
      </section>
    </div>
